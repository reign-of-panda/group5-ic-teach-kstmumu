...
def decision_trees(dF, dF_out, combinatorial = True, peaking = True):
        if combinatorial:
            dF, dF_out = combinatorial_tree(dF, dF_out)
        if peaking:
            dF, dF_out = peaking_trees(dF, dF_out)
        return dF, dF_out
...
def peaking_trees(dF, dF_filtered_out):
    model = None
...
def acceptance(folder_path, file_name):
    ...
    dF_acc['dilepton_mass'] = Mass(dF_acc['mu_minus_PE'], dF_acc['mu_minus_P']) + Mass(dF_acc['mu_plus_PE'], dF_acc['mu_plus_P'])
    ...
    dF_acc, dF_acc_filtered_out = decision_trees(dF_acc, dF_acc_filtered_out, True, True)
