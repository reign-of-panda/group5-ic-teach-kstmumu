...
class LHCb:
    ...
    def decision_trees(self, combinatorial = True, peaking = True):
        if combinatorial:
            self.combinatorial_tree()
        if peaking:
            self.peaking_trees()
    
    def combinatorial_tree(self):
        model = None
       ...
       'B0_OWNPV_Y', 'B0_OWNPV_Z', 'B0_FD_OWNPV', 'q2', 'phi', 'costhetal',
       'costhetak', 'accept_kaon', 'accept_pion', 'accept_muon',
       'dilepton_mass', 'Phi_M', 'lambda1_M', 'lambda2_M'])
        to_drop = set(self.dF.columns) - to_leave
        
        predict_df = self.dF.drop(to_drop, axis = 1)
        
        res = model.predict(predict_df)
        self.dF["combinatorial_res"] = res
        self.dF_filtered_out = pd.concat([self.dF_filtered_out,
                                          self.dF[self.dF["combinatorial_res"] == 0]],
                                             ignore_index=True).drop_duplicates()
        self.dF = self.dF[self.dF["combinatorial_res"] == 1]
        self.dF = self.dF.drop("combinatorial_res", axis = 1)
    
    def peaking_trees(self):
        model = None
        
        path = "trees/"
        
        to_leave = set(['mu_plus_MC15TuneV1_ProbNNk', 'mu_plus_MC15TuneV1_ProbNNpi',
       ...
       'costhetak'])
        ...
        trees = ["jpsi", "jpsi_mu_k_swap", "jpsi_mu_pi_swap", "k_pi_swap", "phimumu", "pKmumu_piTok_kTop",\
                 "pKmumu_piTop", "psi2S"]
        for tree in trees:
            ...
            res = model.predict(predict_df)
            self.dF[tree + "_res"] = res
            self.dF_filtered_out = pd.concat([self.dF_filtered_out,
                                              self.dF[self.dF[tree + "_res"] == 0]],
                                                 ignore_index=True).drop_duplicates()
            self.dF = self.dF[self.dF[tree + "_res"] == 1]
            self.dF = self.dF.drop(tree + "_res", axis = 1)
    ...
    def intermediary_plotting(self):
        ...
        print("FIT PARAMETERS are: ", params2)
    ...
    def run_analysis(self):
        ...
        # Decision Trees applied to data
        self.decision_trees(True, True) # first param to apply combinatorial DT, second to apply peaking bg decision trees
